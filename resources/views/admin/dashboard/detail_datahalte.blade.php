<div
    ...
    <hr>
    <p><strong>Kendala Halte:</strong></p>
    <ul>
        @forelse ($kendala as $item)
            <li>
                {{ $item->kendalaHalte->kendala_halte ?? 'Kendala tidak ada' }}
            </li>
        @empty
    ...
    <div class="image-container">
        @if ($data->bukti_kendala_halte)
            <img src="{{ asset('storage/' . $data->bukti_kendala_halte) }}" alt="Bukti Kendala">
        @else
            Kendala tidak ada
        @endif
